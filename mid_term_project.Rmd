...
```{r }
set.seed(1000)
reg.fwd=regsubsets(Life.expectancy ~ Adult.Mortality + infant.deaths + Alcohol + percentage.expenditure + Hepatitis.B + 
                Measles + BMI + under.five.deaths + Polio + Total.expenditure + Diphtheria + HIV.AIDS + GDP + 
                Population + thinness..1.19.years + thinness.5.9.years + Income.composition.of.resources + 
                Schooling, data=train,method="forward",nvmax=100)
...
par(mfrow=c(1,1))
plot(1:18,testASE,type="l",xlab="# of predictors",ylab="test vs train ASE",ylim=c(0.3,0.8))
index<-which(testASE==min(testASE))
points(index,testASE[index],col="red",pch=10)
rss<-summary(reg.fwd)$rss
lines(1:18,rss/100,lty=3,col="blue")  #Dividing by 100 since ASE=RSS/sample size
```
...
control <- trainControl(method="repeatedcv", number=10, repeats=3)
set.seed(100)
model.knn <- train( Life.expectancy ~ Adult.Mortality +  Alcohol + Total.expenditure + HIV.AIDS + Income.composition.of.resources + thinness..1.19.years, data=who_imp[,-c(2)], method="knn", trControl=control)
#Top 10 predictor ranking
importance.knn <- varImp(model.knn, scale=FALSE)
rank.knn <- importance.knn$importance
write.csv(rank.knn, "rank.knn.csv")
...
{
   trainIndices = sample(1:dim(who_imp)[1],round(splitPerc * dim(who_imp)[1]))
    train = who_imp[trainIndices,]
    test = who_imp[-trainIndices,]
  for(i in 1:numks)
  {
    classifications = knn(train[,c(5:21)],test[,c(5:21)],train$Life.expectancy, prob = TRUE, k = i)
...
Model1_fit = lm(Life.expectancy ~ Adult.Mortality + Alcohol + Total.expenditure + HIV.AIDS + Income.composition.of.resources + thinness..1.19.years, data = train)
sm <- summary(Model1_fit)
MSE <- get_mse(Model1_fit, var.estimate = FALSE)

predictions_model = predict(Model1_fit,test)
predictions_df <- as.data.frame(predictions_model)
predictions_df
sm
rmse(train$Life.expectancy, predictions_model)
```